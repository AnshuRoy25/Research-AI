...
// NEW DOMContentLoaded - REPLACE WITH THIS
...
// Modified createVerifyButton function - REPLACE WITH THIS
function createVerifyButton(ipfsHash) {
    if (!ipfsHash) return null;

    const verifyButton = document.createElement('button');
    verifyButton.className = 'verify-button loading-payment-status';
    verifyButton.innerHTML = '<span class="loading-spinner"></span><span>Checking...</span>';
    verifyButton.title = 'Checking payment status...';
    verifyButton.disabled = true; // Disable during loading
    
    // We'll add the click event listener after payment status check
    return verifyButton;
}
// Display message with proper styling according to CSS
// Modified displayMessage function to check payment status
function displayMessage(sender, content, isUser = false, isLoadingSession = false, ipfsHash = null) {
    ...
    if (!isUser && ipfsHash) {
        ...
        if (verifyButton) {
            ...
            // Check payment status after a short delay to allow Web3 to initialize
            setTimeout(() => checkAndUpdateVerifyButton(messageDiv, ipfsHash), 1000);
        }
    }
...
// Check if user has already paid for verification when loading messages
async function checkAndUpdateVerifyButton(messageDiv, ipfsHash) {
    if (!ipfsHash) return;
    
    const verifyButton = messageDiv.querySelector('.verify-button');
    if (!verifyButton) return;
    
    try {
        // Show loading state
        verifyButton.classList.add('loading-payment-status');
        verifyButton.innerHTML = '<span class="loading-spinner"></span><span>Checking...</span>';
        ...
        if (hasPaid) {
            // User has paid
            verifyButton.classList.remove('loading-payment-status');
            verifyButton.classList.add('paid');
            verifyButton.innerHTML = '<span>Verify (Paid)</span>';
            verifyButton.title = 'Click to view verified response on IPFS (Payment confirmed)';
            verifyButton.disabled = false;
            
            // Add click event for paid button
            verifyButton.addEventListener('click', async () => {
                verifyButton.style.opacity = '0.7';
                verifyButton.style.transform = 'scale(0.98)';
                
                // Direct to IPFS since payment is confirmed
            ...
            verifyButton.title = 'Verify this response on the blockchain for authenticity and immutability';
            verifyButton.disabled = false;
            
            // Add click event for unpaid button
            verifyButton.addEventListener('click', async () => {
                verifyButton.style.opacity = '0.7';
                verifyButton.style.transform = 'scale(0.98)';
                
                await verifyWithPayment(ipfsHash);
        ...
        // On error, show normal verify button
        verifyButton.classList.remove('loading-payment-status');
        verifyButton.innerHTML = '<span>Verify</span>';
        verifyButton.title = 'Verify this response on the blockchain for authenticity and immutability';
        verifyButton.disabled = false;
        
        // Add click event for error case
        verifyButton.addEventListener('click', async () => {
            verifyButton.style.opacity = '0.7';
            verifyButton.style.transform = 'scale(0.98)';
